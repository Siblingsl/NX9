...
function sortForArrangeGrid(
  ...
  const compareByVisualIndex = (a: string, b: string) =>
    (visualIndex.get(a) ?? Number.MAX_SAFE_INTEGER) - (visualIndex.get(b) ?? Number.MAX_SAFE_INTEGER);
...
export function applyNodeAlignment(
  ...
  if (action === 'distribute-x') {
    ...
    const packsVertically =
      action === 'align-left' || action === 'align-center-x' || action === 'align-right';
    const packsHorizontally =
      action === 'align-top' || action === 'align-center-y' || action === 'align-bottom';
    for (const node of selected) {
      const rect = rectById.get(node.id)!;
      const pos = { ...(node.position || { x: 0, y: 0 }) };
