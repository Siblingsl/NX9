...
import { readVideoGenMode, patchVideoGenMode, type VideoGenMode } from './video-gen-modes';
...
export interface VideoWorkspaceToolbarProps {
  blockId: string;
...
export function VideoWorkspaceToolbar({
  ...
  return (
    ...
    >
      <VideoGenModeChip
        mode={videoGenMode}
        onChange={(mode: VideoGenMode) => patch(patchVideoGenMode(mode))}
      />

      <span className="w-px h-3.5 bg-line/50 mx-0.5" />

      <VideoParamChips blockId={blockId} onPatch={patch} />
        ...
        <div className="px-3 py-2.5 space-y-2" onMouseDown={stop}>
          ...
          <label className="block space-y-1">
            ...
            <input
              ...
              onChange={(e) => patch({ modelParams: e.target.value || undefined })}
