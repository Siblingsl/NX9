...
export interface VideoGenModeChipProps {
  mode: VideoGenMode;
  onChange: (mode: VideoGenMode) => void;
}

export function VideoGenModeChip({ mode, onChange }: VideoGenModeChipProps) {
  const btnRef = useRef<HTMLButtonElement>(null);
  const [open, setOpen] = useState(false);
  const current = lookupVideoGenMode(mode);
  ...
  return (
    <>
      ...
      <VideoPopover
        ...
        width={168}
      >
        <p className="px-3 pt-2 pb-1 text-[10px] text-ink/40">视频生成模式</p>
            ...
            <button
              ...
              className={`w-[calc(100%-8px)] flex items-center gap-2 px-2.5 py-1.5 mx-1 rounded-lg text-[11px] transition-colors ${
                active
                  ...
                  : 'text-ink/65 hover:bg-surface/80'
              }`}
            >
