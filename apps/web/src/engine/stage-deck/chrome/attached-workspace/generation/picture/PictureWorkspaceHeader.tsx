...
export interface PictureWorkspaceHeaderProps {
  kind: string;
...
export function PictureWorkspaceHeader({
  kind,
  status,
  model,
  onModelChange,
  onCollapse,
}: PictureWorkspaceHeaderProps) {
  const meta = lookupBlock(kind);
  const modelBtnRef = useRef<HTMLButtonElement>(null);
  const [modelOpen, setModelOpen] = useState(false);
  const modelLabel = PICTURE_GEN_MODELS.find((m) => m.id === model)?.label ?? model;
