...
export function LibraryWorkflowSubPanel() {
    ...
    <div className="space-y-3 text-xs">
      <section className="space-y-2">
        ...
        <button
          ...
          className="w-full rounded-xl border border-line py-2 hover:border-brand/40 disabled:opacity-50 text-xs"
        >
          导出选中模块
        </button>
        <button
          ...
          className="w-full rounded-xl bg-brand text-white py-2 hover:bg-brand/90 disabled:opacity-50 text-xs"
        >
          导出整个画布
        </button>
      </section>
      ...
      <section className="space-y-2">
        ...
        <button
          ...
          className="w-full rounded-xl border border-warn/40 text-warn py-2 hover:bg-warn/5 disabled:opacity-50 text-xs"
        >
          替换画布（可撤销）
        </button>
      </section>
        ...
        <ul className="space-y-2 max-h-48 overflow-y-auto nx9-scroll">
          {WORKFLOW_TEMPLATES.map((tpl) => (
            <li key={tpl.id} className="rounded-lg border border-line p-2">
              <p className="font-medium text-ink text-xs">{tpl.label}</p>
              <p className="text-[10px] text-ink/40 mt-0.5 line-clamp-2">{tpl.description ?? ''}</p>
