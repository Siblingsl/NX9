...
export function useAssetLibraryGeneration(deps: AssetLibraryGenerationDeps) {
  ...
  const handleUploadCharacterView = useCallback(
    async (file: File, char: CharacterProfile, view: string) => {
      try {
        ...
        if (!res.url?.trim()) throw new Error('角色视图上传失败或未返回 URL，禁止空成功');
        ...
        const msg = e instanceof Error ? e.message : '角色视图上传失败，禁止空成功';
        toastError(msg);
        appendLog(msg);
      }
    },
    [appendLog, saveCharacter],
  );
  ...
  const uploadCroppedEntityCover = useCallback(
    async (sheetUrl: string, kind: EntitySheetCropKind, fileStem: string): Promise<string> => {
      ...
      if (!uploaded.url?.trim()) throw new Error('封面裁切上传失败或未返回 URL，禁止空成功');
      return uploaded.url;
    },
    [],
  );
  ...
  const cropWorkspaceEntityCover = useCallback(
    async (item: BacklotWorkspaceItem) => {
      ...
      try {
        ...
        const msg = `封面裁切失败 · ${item.label}: ${String(e)}`;
        appendLog(msg);
        toastError(msg);
      } finally {
  ...
  const generateCostumeSheets = useCallback(
    async (items: BacklotWorkspaceItem[]) => {
      if (scope !== 'private') {
        const msg = '服装设定板：请先导入到私有项目库再生成，禁止空成功';
        appendLog(msg);
        toastError(msg);
        return;
      }
      if (!canEditPrivate) {
        const msg = '服装设定板：当前项目不可编辑，禁止空成功';
        appendLog(msg);
        toastError(msg);
        return;
      }
      ...
      if (targets.length === 0) {
        const msg = '服装设定板：没有可生成的服装条目，禁止空成功';
        appendLog(msg);
        toastError(msg);
        return;
      }
      ...
      for (let i = 0; i < targets.length; i++) {
        ...
        try {
          ...
          if (!imageUrl) throw new Error('未返回图片，禁止空成功');
      ...
      if (fail > 0) toastError(`服装设定板失败 ${fail}/${targets.length}`);
    },
  ...
  const generateSceneSheet = useCallback(
    async (item: BacklotWorkspaceItem) => {
      ...
      try {
        ...
        toastError(`场景设定板失败 · ${item.label}: ${msg}`);
      } finally {
  ...
  const generatePropSheet = useCallback(
    async (item: BacklotWorkspaceItem) => {
      ...
      try {
        ...
        toastError(`道具设定板失败 · ${item.label}: ${msg}`);
      } finally {
  ...
  const generateCharacterMasterSheet = useCallback(
    async (char: CharacterProfile) => {
      ...
      try {
        ...
        if (!sheetUrl) throw new Error('完整设定板未返回图片，禁止空成功');
  ...
  const generateCharacterCategorySheets = useCallback(
    async (char: CharacterProfile) => {
      ...
      if (!masterUrl) {
        const msg = '五类原图：请先生成并确认角色完整设定板，禁止空成功';
        appendLog(msg);
        toastError(msg);
        return;
      }
      ...
      try {
        ...
        for (let categoryIndex = 0; categoryIndex < CHARACTER_SHEET_CATEGORY_LAYOUTS.length; categoryIndex++) {
          ...
          const sheetUrl = urls[0];
          if (!sheetUrl) throw new Error(`${category.label}未返回图片，禁止空成功`);
          ...
          for (let i = 0; i < entries.length; i++) {
            ...
            if (!uploaded.url?.trim()) {
              throw new Error(`${category.label}面板 ${panelId} 上传失败或未返回 URL，禁止空成功`);
            }
  ...
  const handleUploadWorkspaceMedia = useCallback(
    async (file: File, item: BacklotWorkspaceItem, field: string) => {
      try {
        const res = await api.uploadAsset(file);
        if (!res.url?.trim()) throw new Error('素材上传失败或未返回 URL，禁止空成功');
        ...
        const msg = e instanceof Error ? e.message : '素材上传失败，禁止空成功';
        toastError(msg);
        appendLog(msg);
      }
    },
  ...
  const handleUploadAudio = useCallback(
    async (file: File, target: { kind: 'character' | 'sound'; id: string }) => {
      try {
        const res = await api.uploadAsset(file);
        if (!res.url?.trim()) throw new Error('音频上传失败或未返回 URL，禁止空成功');
        ...
        const msg = e instanceof Error ? e.message : '音频上传失败，禁止空成功';
        toastError(msg);
        appendLog(msg);
      }
    },
    [appendLog, scope, characters, publicCharacters, sounds, publicSounds, saveCharacter, saveSound],
  );
