  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const prompt = enrichPromptWithCharacters(
        `${base}${aspect !== '16:9' ? `, aspect ratio ${aspect}` : ''}${durationSec ? `, ${durationSec}s clip` : ''}`.trim(),
        activeCharacters,
      );
    ...
    <BlockShell {...props}>
      <div className="space-y-2 text-sm">
        ...
        <div className="grid grid-cols-2 gap-2">
          <select
            value={aspect}
            onChange={(e) => updateNodeData(props.id, { aspect: e.target.value })}
            className="rounded-lg border border-line bg-white px-2 py-1 text-xs"
          >
            {CLIP_GEN_ASPECTS.map((a) => (
          ...
          <label className="flex items-center gap-1 text-[10px] text-ink/60">
            ...
            <input
              ...
              value={durationSec}
              onChange={(e) =>
                updateNodeData(props.id, { durationSec: Number(e.target.value) || 5 })
              }
