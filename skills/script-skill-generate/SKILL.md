...
- `bible.world.visualStyleNotes` 必须已经选择，且只能使用明确的全片视觉风格（真人写实、写实 3D、风格化 3D、二维动漫、国漫水墨、定格动画等）；未选择时禁止生成。
...
2. 校验并锁定 `bible.world.visualStyleNotes`；全片所有人物、场景和后续视觉资产必须遵守同一风格，不得在生成中途切换真人/3D/二维等形态。
3. 锁定本轮集数：用户指定 > brief.episodeCount > 默认 1。
4. 若已有成稿：提取上一集结尾做衔接；提取其场景头/对白形态做体例对齐（若旧稿不规范，改用本 skill 标准体例，但剧情仍衔接）。
5. 为每一集规划：开场钩子 → 冲突升级 → 集末钩子；每集 2–5 场。
6. 按标准体例写 `bodyMd`；组装 `patch`；`explanation` 一句话。
7. 自检清单全部通过后再输出。
...
- [ ] `bible.world.visualStyleNotes` 已选择并在本轮保持不变
- [ ] 每集 `title` 形如 `第N集 短标题`
