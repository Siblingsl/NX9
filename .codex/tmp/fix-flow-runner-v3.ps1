...
'@
$forkBlock = $forkRaw -split "`r?`n"

# Anchor: the orphan tail of the truncated variant-fork block.
$startMarker = "      output: upstream.prompts?.[0],"
$endMarker = "  if (kind === 'prompt-diff') {"
$startIdx = -1
$endIdx = -1
for ($i = 0; $i -lt $lines.Length; $i++) {
  if ($lines[$i].TrimEnd() -eq $startMarker) { $startIdx = $i; break }
}
for ($i = 0; $i -lt $lines.Length; $i++) {
  if ($lines[$i].TrimEnd() -eq $endMarker) { $endIdx = $i; break }
}
if ($startIdx -lt 0) { throw 'variant-fork orphan start not found' }
