...
$oldPara = "- `NX9-REAL-COMPLETION-LEDGER.md` F-046～050：按第一～五批实际工作树重新定性，F-046（HyperFrames 取消不得成功）、F-047/048/049（BGM、视频、剪辑）已修，F-050 智能剪辑门禁已修；LEDGER 本身的历史数字由人工在下一轮滚动时统一重算，本轮已避免在其上叠加新结论。"
$newPara = @(
...
if (-not $text.Contains($oldPara)) { throw 'LEDGER paragraph not found' }
$text = $text.Replace($oldPara, $newPara)

[System.IO.File]::WriteAllText($path, $text, (New-Object System.Text.UTF8Encoding($false)))
Write-Output 'deep log patch OK'
