...
$o1 = "      const result = await consumeDirectorKeyframeBatch({" + $nl + "        batch: directorBatch," + $nl + "        chain: sourceChain,"
$n1 = "      const result = await consumeDirectorKeyframeBatch({" + $nl + "        batch: directorBatch," + $nl + "        chain: sourceChain," + $nl + "        // DD-D-09: 每镜成功立即写回链镜表，中断后不丢已成功镜头。" + $nl + "        onShotProgress: (shotId, shotPatch) => {" + $nl + "          const latest = readChainStoryboard(sourceChainNode.data as Record<string, unknown>);" + $nl + "          if (!latest) return;" + $nl + "          updateNodeData(directorBatch.sourceChainDeskId, {" + $nl + "            chainStoryboard: {" + $nl + "              ...latest," + $nl + "              shots: latest.shots.map((s) => s.id === shotId ? { ...s, ...shotPatch } : s)," + $nl + "            }," + $nl + "          });" + $nl + "        },"
Replace-One $p1 $o1 $n1 'flow onShotProgress'
