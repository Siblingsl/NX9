...
export {
  ...
  type BuildCameraMovePromptOptions,
...
export {
  ...
  type ConnectedAudioModelOption,
...
export {
  ...
  type VideoGenModelGroup,
  type BuiltinVideoModelOption,
} from './data/video-gen-models';
export {
  LLM_MODELS,
  lookupLlmModel,
  isBuiltinLlmModelId,
...
export {
  ...
  lookupAudioModel,
  isBuiltinAudioModelId,
  lookupAudioVoice,
  isKnownAudioVoiceId,
  type AudioModelDef,
  type AudioModelGroup,
  type AudioVoiceDef,
} from './data/audio-models';
export {
  BUILTIN_GROUP_LABEL,
  ...
  isBuiltinVideoModelValue,
  type MergedModelOption,
  type ModelOptionSource,
  type ModelConnectionLike,
...
// 多格推演（multi-grid）：多机位 9/25 宫格 · 剧情四宫格 · 画面推演
export type {
  MultiGridMode,
  MultiGridCell,
  MultiGridPlan,
...
export {
  ...
  buildMultiGridCellVideoPrompt,
  planCellsToGridCellPrompts,
  planToGridReverseResult,
} from './utils/multi-grid-plan';

// 多格推演 → 分镜镜头映射（增量新增：镜头层面的生成与写回，与图层面回写分工）
export type {
  MultiGridShotWarningCode,
  MultiGridShotWarning,
  MultiGridTargetEpisode,
  MultiGridShotIdInput,
  MultiGridCellsToShotsOptions,
  MultiGridShotPlanResult,
  MultiGridShotPreviewRow,
  MultiGridBreakdownConvertOptions,
  MultiGridBreakdownInsertResult,
} from './utils/multi-grid-to-shots';
export {
  MULTI_GRID_SHOT_ID_PREFIX,
  MULTI_GRID_SHOT_WRITEBACK_KEY_PREFIX,
  MULTI_GRID_SHOT_DEFAULT_DURATION_SEC,
  MULTI_GRID_STORY_BEAT_DURATIONS_SEC,
  MULTI_GRID_SHOT_MAX_DURATION_SEC,
  MULTI_GRID_SHOT_TYPE_LABELS_ZH,
...
export type {
  ...
  CameraMoveTimelineIssueCode,
  CameraMoveTimelineIssueSeverity,
  CameraMoveTimelineSample,
  CameraMoveTimelineValidation,
  ComposedMovePrompt,
  ComposedMovePromptPart,
...
export {
  ...
  type BuildComposedMovePromptOptions,
  type MoveTimelineCurve,
  type MoveTimelineCurvePoint,
  type MoveTimelineFromShotDurationOptions,
  type MoveTimelineSeed,
  type NormalizeMoveTimelineOptions,
  type SnapMoveTimelineToBeatsOptions,
  type WithMoveTimelinePromptOptions,
} from './utils/camera-move-timeline';

// ── 运镜提示词反向解析 + 两通道合流（增量追加：只读，不改既有正向输出格式） ──
export {
  ...
  type ExtractedMovePrompt,
  type ExtractedMovePromptLine,
  type MovePhraseHit,
  type MovePhraseResolution,
  type ParsedCameraMovePrompt,
  type ParsedComposedMovePart,
  type ParsedComposedMovePrompt,
  type PhraseHit,
  type ScannablePhrase,
  type StrippedCameraMovePrefix,
} from './utils/camera-move-parse';
export {
  DIRECTOR3D_CAMERA_MOVE_PHRASES,
  findDirector3dMovePhrases,
  mergeCameraMoveChannels,
  mergeShotCameraMoveChannels,
  type CameraMoveChannelInfo,
  type CameraMoveChannelSource,
  type CameraMoveMergeInput,
  type CameraMoveMergeResult,
  type CameraMoveShotLike,
  type Director3dMoveHit,
} from './utils/camera-move-merge';

// ── 音频节拍网格 → 时间轴 / 多格时长（增量追加：消费既有 beat-analyze 服务，不改既有导出） ──
export {
  beatGridEndSec,
  ...
  type BeatSnapResult,
  type BeatSnapStrategy,
  type CellDurationPlan,
  type DistributeSegmentBoundariesOptions,
  type PlanCellDurationsOptions,
  type SnapTimelineToBeatGridOptions,
} from './utils/beat-grid-plan';

// ── 角色设定表 / 三视图（增量追加：消费既有角度 / 表情 / 动作预设，不改既有导出） ──
export type {
  CharacterSheetCell,
...
export {
  ...
  isCharacterSheetConsistencyLevel,
  ...
  type CharacterSheetConsistencyDef,
} from './utils/character-sheet-plan';

// ── 跨格 / 跨镜一致性校验（增量追加：与既有 consistency-repair / script-consistency 不重名，不改既有导出） ──
export type {
  BuildConsistencyReportOptions,
  CompareFacePresenceOptions,
  CrossCellAppearanceCategory,
  CrossCellAppearanceKeyword,
  CrossCellAppearanceTerm,
  CrossCellEvidence,
  CrossCellEvidenceEntry,
  CrossCellFace,
  CrossCellInput,
  CrossCellIssue,
  CrossCellIssueCode,
  CrossCellPickCandidate,
  CrossCellPickResult,
  CrossCellReport,
  CrossCellSeverity,
  ...
  PickBestCellOptions,
} from './utils/consistency-report';
export {
  CONSISTENCY_BASELINE_MAJORITY_RATIO,
  CONSISTENCY_BASELINE_MIN_SAMPLE,
  CONSISTENCY_LOW_CONFIDENCE,
  CONSISTENCY_MIN_COMPARABLE_CELLS,
  CONSISTENCY_OUTFIT_ADJACENCY,
  CROSS_CELL_APPEARANCE_CATEGORY_LABELS,
  CROSS_CELL_APPEARANCE_TERMS,
  CROSS_CELL_SEVERITY_ORDER,
  buildConsistencyReport,
  compareFacePresence,
  crossCellSeverityRank,
...
export {
  ...
  type PictureSizeCandidate,
  type PictureSizePatch,
} from './utils/preset-entrypoints';
export {
  BLOCKING_LAYOUT_IDS,
  BLOCKING_LINE_SPACING,
  BLOCKING_DIALOGUE_SPACING,
  ...
  type BlockingCameraPatch,
} from './utils/blocking-layout';
export {
  ...
  type CapabilitySelfcheckCheck,
  type CapabilitySelfcheckCounts,
  type CapabilitySelfcheckReport,
  type CapabilitySelfcheckSeverity,
} from './utils/capability-selfcheck';

// 逐帧拉片（frame-study）：参考视频逐帧拆解为时间码 + 逐帧反推提示词
export type {
  FrameStudyMode,
...
export {
  ...
  FRAME_STUDY_DEFAULT_COUNT,
  FRAME_STUDY_DEFAULT_INTERVAL_SEC,
  FRAME_STUDY_DEFAULT_ASPECT,
  FRAME_STUDY_SERVER_INTERVALS,
  FRAME_STUDY_SHOT_ID_PREFIX,
  FRAME_STUDY_SHOT_WRITEBACK_KEY_PREFIX,
...
} from './utils/frame-study-plan';
export type {
  FrameStudyShotIdInput,
  FrameStudyShotsOptions,
