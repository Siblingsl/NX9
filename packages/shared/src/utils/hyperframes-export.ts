...
export function timelineToHyperFramesVars(timeline: TimelinePayload): Record<string, unknown> {
  return {
...
export function timelineToHyperFramesHtml(
  timeline: TimelinePayload,
  templateId?: string,
): string {
  const vars = timelineToHyperFramesVars(timeline);
  const clipsHtml = (vars.clips as Array<Record<string, unknown>>)
  ...
  const subtitlesHtml = (vars.clips as Array<Record<string, unknown>>)
...
export function listHyperFramesTemplates(): HyperFramesTemplate[] {
  return [
