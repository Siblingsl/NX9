...
export interface ClipChainItem {
  index: number;
  shotId?: string;
  label: string;
  prompt: string;
...
export interface ClipChainState {
  version: 1;
  items: ClipChainItem[];
  currentIndex: number;
  projectGoal?: string;
...
export function summarizeClipResult(item: ClipChainItem): string {
