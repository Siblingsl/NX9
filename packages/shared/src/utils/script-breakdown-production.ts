...
export function normalizeScriptBreakdownConfig(
  input?: Partial<ScriptBreakdownConfig> | null,
...
export function normalizeScriptBreakdownPrompts(
  input?: Partial<ScriptBreakdownPromptTemplates> | null,
...
function balancedChunks(text: string, count: number): string[] {
  const units = text
    .split(/\n{2,}|(?<=[。！？!?])\s*/)
    .map((item) => item.trim())
    .filter(Boolean);
...
export function splitSourceIntoEpisodeChunks(
  sourceText: string,
  configInput?: Partial<ScriptBreakdownConfig>,
): ScriptEpisodeChunk[] {
...
export function splitLongEpisodeText(text: string, maxChars = 10000): Array<{ text: string; contextBefore?: string }> {
...
export function createScriptBreakdownPromptPack(
  config?: Partial<ScriptBreakdownConfig>,
...
export function parseScriptBreakdownPromptPack(raw: unknown): ScriptBreakdownPromptPack | null {
  if (!raw || typeof raw !== 'object') return null;
  const value = raw as Partial<ScriptBreakdownPromptPack>;
  if (value.schema !== 'nx9-script-breakdown-prompt') return null;
  return createScriptBreakdownPromptPack(value.config, value.prompts);
}

export function createScriptBreakdownExportEnvelope(payload: ScriptBreakdownPayload): ScriptBreakdownExportEnvelope {
  return { schema: 'nx9-script-breakdown-result', version: 1, exportedAt: new Date().toISOString(), payload };
}

export function parseScriptBreakdownExportEnvelope(raw: unknown): ScriptBreakdownPayload | null {
  if (!raw || typeof raw !== 'object') return null;
  const value = raw as Partial<ScriptBreakdownExportEnvelope>;
  const payload = value.schema === 'nx9-script-breakdown-result' ? value.payload : raw as ScriptBreakdownPayload;
  if (!payload || !Array.isArray(payload.episodes)) return null;
  if (validateScriptBreakdownPayload(payload).some((item) => item.level === 'error')) return null;
  return payload;
}

/** 导入与 AI 返回共用的生产质量检查，不再只判断 descriptionZh 非空。 */
export function validateScriptBreakdownPayload(payload: ScriptBreakdownPayload): ScriptBreakdownDiagnostic[] {
  ...
  for (const episode of payload.episodes) {
    ...
    if (!episode.title?.trim()) diagnostics.push({ level: 'error', code: 'episode_title_empty', episodeId: episode.id, message: `${episode.id} 缺少标题` });
    if (!episode.shots.length) diagnostics.push({ level: 'error', code: 'shots_empty', episodeId: episode.id, message: `${episode.title || episode.id} 没有镜头` });
    const sceneIds = new Set(episode.shots.map((shot) => shot.sceneId).filter(Boolean));
    if (sceneIds.size === 0) diagnostics.push({ level: 'warning', code: 'scenes_missing', episodeId: episode.id, message: `${episode.title} 缺少场景归属` });
    for (const shot of episode.shots) {
      if (!shot.id || shotIds.has(shot.id)) diagnostics.push({ level: 'error', code: 'shot_id_invalid', episodeId: episode.id, message: `${episode.title} 存在缺失或重复镜头 ID` });
      shotIds.add(shot.id);
