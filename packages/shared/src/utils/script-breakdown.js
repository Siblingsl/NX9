...
function extractCharacters(text, dialogue) {
...
function makeShot(episodeId, episodeIndex, shotIndex, sceneCode, text) {
    ...
    const audiovisualLanguage = `稳定镜头贴近情境，中景到特写逐步交代${subject}。光色层次与角色状态形成对比，强化这一拍的情绪冲击。`;
    return {
...
function splitEpisodeChunks(sourceText) {
    const matches = [...sourceText.matchAll(EP_RE)];
    if (matches.length === 0)
        return [{ index: 1, title: '第一集', text: sourceText }];
...
function splitShotChunks(text) {
    const lines = text.split('\n').map((line) => line.trim()).filter(Boolean);
    const chunks = [];
    let current = [];
    for (const line of lines) {
        const isSceneHead = /^分镜\s*\d+/.test(line) ||
            /^镜头\s*\d+/.test(line) ||
            /^\d+\s*[.、]/.test(line) ||
...
export function buildScriptBreakdownFromText(sourceText) {
    const source = sourceText.trim();
    const title = source.match(/[《「]([^》」]+)[》」]/)?.[1]?.trim() || '剧本拆分';
    const episodes = splitEpisodeChunks(source).map((ep, epPos) => {
        const episodeId = `ep-${ep.index}`;
        const shots = splitShotChunks(ep.text).map((chunk, i) => makeShot(episodeId, ep.index, i + 1, `${ep.index}-${i + 1}`, chunk));
        return {
            id: episodeId,
