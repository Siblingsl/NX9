...
const SHOT_TYPE_MAP: Record<string, ShotType> = {
  特写: 'close',
  close: 'close',
  'close-up': 'close',
  近景: 'close',
...
export function parseStoryboardMarkdown(markdown: string): StoryboardShot[] {
  ...
  for (let r = 1; r < tableLines.length; r++) {
    ...
    const get = (idx: number, fallback = '') => (idx >= 0 && idx < cells.length ? cells[idx] : fallback);
