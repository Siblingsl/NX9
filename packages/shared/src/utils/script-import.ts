import type { StoryboardShot, ShotType } from '../types/storyboard';

export interface ParsedScriptBackground {
  title: string;
  outline: string;
...
const SCENE_HEAD_RE =
  /^\*{0,2}(\d+-\d+)\s*(日|夜|晨|暮|黄昏|清晨)?\s*(内|外)?\s*(.+?)\*{0,2}\s*$/;
...
export function parseChineseScript(fullText: string): {
  background: ParsedScriptBackground;
  ...
  const biosMatch = fullText.match(
  ...
  const genre =
    /武侠|仙侠|科幻|悬疑|爱情|商战|宫斗/.exec(`${outline}\n${characterBios}`)?.[0] ?? '';

  const episodes: { num: number; start: number; end: number }[] = [];
  ...
  for (const { ep, text } of chunks) {
    ...
    let current: ParsedScriptScene | null = null;
    const buf: string[] = [];

    const flush = () => {
      if (!current) return;
      current.content = buf.join('\n').trim();
      if (current.content) scenes.push({ ...current });
      buf.length = 0;
    };

    for (const raw of lines) {
      const line = raw.trim();
      if (!line || /^第\s*[一二三四五六七八九十\d]+\s*集/.test(line)) continue;
...
function guessShotType(content: string): ShotType {
  if (/特写|close/i.test(content)) return 'close';
  if (/全景|远景|wide/i.test(content)) return 'wide';
  if (/大全景|extreme/i.test(content)) return 'extreme-wide';
  return 'medium';
}

/** 将解析场景转为故事板镜头（规则合成，可再由 LLM 精修） */
export function scenesToStoryboardShots(scenes: ParsedScriptScene[]): StoryboardShot[] {
