import type { TimelinePayload, TimelineClip } from '../types/timeline';
...
function mapClipType(type: TimelineClip['type']): 'video' | 'audio' | 'image' {
  if (type === 'subtitle' || type === 'overlay') return 'image';
  return type;
}
...
export function timelineToRemotionInputProps(timeline: TimelinePayload): { timeline: TimelinePayload } {
  return { timeline };
}

/** Remotion Studio 工程包描述（用于生成 ZIP） */
export interface RemotionStudioBundle {
  zipFilename: string;
  files: Array<{ name: string; content: string }>;
}

/** 生成 Remotion Studio 工程包描述（composition 源码 + inputProps.json + README） */
export function timelineToRemotionStudioBundle(
  timeline: TimelinePayload,
): RemotionStudioBundle {
  const ts = Date.now();
